...
public class DialogRuleCarRoulette {
    private Context context;
    Callback callback;
    private static DialogRuleCarRoulette mInstance;

    int[] rummy_rules = {
            R.drawable.ic_ab_rule4
    };

    public static DialogRuleCarRoulette getInstance(Context context) {
        if (null == mInstance) {
            synchronized (DialogRuleCarRoulette.class) {
                if (null == mInstance) {
                    mInstance = new DialogRuleCarRoulette(context);
                }
            }
        }
     ...
     */
    public DialogRuleCarRoulette init(Context context) {
        try {
    ...
    private DialogRuleCarRoulette initDialog() {
        dialog = Functions.DialogInstance(context);
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        dialog.setTitle("");
        dialog.setContentView(R.layout.dialog_rulescarroulette);
    ...
    public DialogRuleCarRoulette(Context context) {
        this.context = context;
    }

    public DialogRuleCarRoulette() {
    }

    Dialog dialog;

    public DialogRuleCarRoulette show() {
